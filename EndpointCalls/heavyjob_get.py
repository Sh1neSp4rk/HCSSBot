...
def get_heavyjob_data():
    url = "https://api.hcssapps.com/heavyjob/api/v1/data"
    token = get_token()
    if not token:
    ...
    start_time = log_function_call("get_heavyjob_data")
    response = requests.get(url, headers=headers)
    logging.info(f"Response code for heavy job data: {response.status_code}")

    if response.status_code != 200:
        logging.error(f"Error fetching heavy job data: {response.status_code}")
        log_function_completion("get_heavyjob_data", start_time)
        return None
